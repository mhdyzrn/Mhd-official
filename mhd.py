...
def log(value, status='INFO', color='[R1]'):
    value = colors('{color}''[CC]''[{time}] [P1]:: {color}{status} [P1]:: {color}{value}[CC]'.format(
        time=datetime.datetime.now().strftime('Mhd OfficiaL'),
        value=value,
        color=color,
        status=status
    ))
...
class inject(object):
    ...
    def start(self):
        try:
            ...
            frontend_domains = open(real_path('/mhd.txt')).readlines()
            frontend_domains = filter_array(frontend_domains)
            if len(frontend_domains) == 0:
                self.log('Frontend Domains not found. Please check mhd.txt', color='[Y1]')
                return
...
class domain_fronting(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.log('STATUS 200 OK...!''[CC]'.format(self.proxy_host, self.proxy_port), color='[CC]')
        except OSError:
...
def main():
    print(colors('\n'.join([
        ...
        '',
        '[CC]'
        '[G1]YOUTUBE CHANNEL', '[CC]'
        '[G1]MHD OFFICIAL', '[CC]'
        '[G1]INTERNET GRATIS ', '[CC]'
        '',
    ])))
